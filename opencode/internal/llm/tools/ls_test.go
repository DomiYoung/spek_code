...
func TestLsTool_Run(t *testing.T) {
	...
	t.Run("handles empty path parameter", func(t *testing.T) {
		// Skip this test because config.WorkingDirectory() panics when config is not loaded
		// This test requires proper config initialization which is not available in unit tests
	...
	t.Run("handles relative path", func(t *testing.T) {
		// Skip this test because config.WorkingDirectory() panics when config is not loaded
		// Relative paths require config initialization to resolve against WorkingDirectory
		t.Skip("Skipping: requires config initialization for relative path resolution")
	})
}
