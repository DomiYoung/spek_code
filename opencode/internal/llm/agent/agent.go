...
func (a *agent) Run(ctx context.Context, sessionID string, content string, attachments ...message.Attachment) (<-chan AgentEvent, error) {
	if !a.provider.Model().SupportsAttachments && len(attachments) > 0 {
		logging.WarnPersist(fmt.Sprintf("Model %s does not support image attachments, images will be ignored", a.provider.Model().Name))
		attachments = nil
	}
