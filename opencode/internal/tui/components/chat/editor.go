...
func (m *editorCmp) View() string {
	...
	// Claude Code style: use UserIcon (›) as prompt with primary color
	style := lipgloss.NewStyle().
		Padding(0, 0, 0, 1).
		Bold(true).
		Foreground(t.Primary())

	// Use Claude Code style prompt icon
	promptIcon := styles.UserIcon

	if len(m.attachments) == 0 {
		return lipgloss.JoinHorizontal(lipgloss.Top, style.Render(promptIcon), m.textarea.View())
	}
	m.textarea.SetHeight(m.height - 1)
	return lipgloss.JoinVertical(lipgloss.Top,
		m.attachmentsContent(),
		lipgloss.JoinHorizontal(lipgloss.Top, style.Render(promptIcon),
			m.textarea.View()),
	)
}
