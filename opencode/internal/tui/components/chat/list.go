...
func (m *messagesCmp) working() string {
	...
	if m.IsAgentWorking() && len(m.messages) > 0 {
		...
		// Claude Code style: status dot + spinner + task description
		statusDot := baseStyle.Foreground(t.Primary()).Render(styles.DotHalfIcon)
		...
		if task != "" {
			text += baseStyle.
				...
				Bold(true).
				Render(fmt.Sprintf("%s %s %s", statusDot, m.spinner.View(), task))
		}
	}
	return text
}
...
func NewMessagesCmp(app *app.App) tea.Model {
	s := spinner.New()
	// Claude Code style: use Dot spinner for cleaner look
	s.Spinner = spinner.Dot
	s.Style = lipgloss.NewStyle().Foreground(lipgloss.Color("#D97757")) // Claude Orange
	vp := viewport.New(0, 0)
