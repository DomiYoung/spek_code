...
func (s *sessionDialogCmp) View() string {
	...
	for i := startIdx; i < endIdx; i++ {
		...
		// Claude Code style: current session indicator
		prefix := "  "
		...
		if i == s.selectedIdx {
			itemStyle = itemStyle.
				...
				Bold(true)
		} else if sess.ID == s.selectedSessionID {
			// Current session highlighted differently
			itemStyle = itemStyle.
				Foreground(t.Primary())
		}

		sessionItems = append(sessionItems, itemStyle.Padding(0, 1).Render(prefix+sess.Title))
	}

	// Claude Code style: list icon
	listIcon := baseStyle.Foreground(t.Primary()).Render("☰")
	...
	title := baseStyle.
		...
		Width(maxWidth).
		Padding(0, 1).
		Render(listIcon + " Switch Session")
