...
func (p *permissionDialogCmp) renderButtons() string {
	...
	// Claude Code style: use semantic colors for permission actions
	// Allow = success (green), Deny = error (red), Session = primary (orange)
	switch p.selectedOption {
	case 0: // Allow selected
		allowStyle = allowStyle.Background(t.Success()).Foreground(t.Background())
		allowSessionStyle = allowSessionStyle.Background(t.Background()).Foreground(t.Primary())
		denyStyle = denyStyle.Background(t.Background()).Foreground(t.Error())
	case 1: // Allow for session selected
		allowStyle = allowStyle.Background(t.Background()).Foreground(t.Success())
		allowSessionStyle = allowSessionStyle.Background(t.Primary()).Foreground(t.Background())
		denyStyle = denyStyle.Background(t.Background()).Foreground(t.Error())
	case 2: // Deny selected
		allowStyle = allowStyle.Background(t.Background()).Foreground(t.Success())
		allowSessionStyle = allowSessionStyle.Background(t.Background()).Foreground(t.Primary())
		denyStyle = denyStyle.Background(t.Error()).Foreground(t.Background())
	}
...
func (p *permissionDialogCmp) renderHeader() string {
	...
	// Claude Code style: tool icon based on tool type
	var toolIcon string
	...
	toolValue := baseStyle.
		...
		Render(fmt.Sprintf(": %s %s", toolIcon, p.permission.ToolName))
...
func (p *permissionDialogCmp) render() string {
	...
	// Claude Code style: warning icon for permission request
	warningIcon := baseStyle.Foreground(t.Warning()).Render("⚠")
	
	title := baseStyle.
		Bold(true).
		Width(p.width - 4).
		Foreground(t.Primary()).
		Render(warningIcon + " Permission Required")
	// Render header
	...
	// Claude Code style: rounded border with primary color
	return baseStyle.
		Padding(1, 0, 0, 1).
