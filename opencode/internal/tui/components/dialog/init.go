...
func (m InitDialogCmp) View() string {
	...
	// Claude Code style: info icon for initialization
	infoIcon := baseStyle.Foreground(t.Info()).Render("ℹ")
	...
	title := baseStyle.
		...
		Width(maxWidth).
		Padding(0, 1).
		Render(infoIcon + " Initialize Project")
	...
	// Claude Code style: success color for positive action
	if m.selected == 0 {
		yesStyle = yesStyle.
	...
	yes := yesStyle.Padding(0, 3).Render("Yes (y)")
	no := noStyle.Padding(0, 3).Render("No (n)")
