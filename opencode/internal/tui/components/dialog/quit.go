...
func (q *quitDialogCmp) View() string {
	...
	// Claude Code style: warning icon for quit confirmation
	...
	// Claude Code style: destructive action uses error/warning colors
	if q.selectedNo {
		noStyle = noStyle.Background(t.Primary()).Foreground(t.Background())
		yesStyle = yesStyle.Background(t.Background()).Foreground(t.Error())
	} else {
		yesStyle = yesStyle.Background(t.Error()).Foreground(t.Background())
		noStyle = noStyle.Background(t.Background()).Foreground(t.Primary())
	}

	yesButton := yesStyle.Padding(0, 2).Render("Yes (y)")
	noButton := noStyle.Padding(0, 2).Render("No (n)")

	buttons := lipgloss.JoinHorizontal(lipgloss.Left, yesButton, spacerStyle.Render("  "), noButton)

	// Claude Code style: title with warning icon
	title := baseStyle.
		Foreground(t.Text()).
		Bold(true).
		Render(warningIcon + " " + question)
	...
	// Claude Code style: rounded border with primary color accent
