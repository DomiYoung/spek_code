...
**追问焦点**：
```
第 1-2 轮：目的 - "这个功能要解决什么问题？"
第 3-4 轮：约束 - "有什么技术/时间/资源限制？"
第 5-6 轮：成功标准 - "怎样算做好了？"
第 7-8 轮：边界情况 - "需要处理哪些特殊情况？"
```
...
**下一步选择**：
1. 📝 保存设计文档到 `docs/plans/YYYY-MM-DD-<topic>-design.md`
2. 🚀 准备好开始实现了吗？
3. 🔄 还需要调整某些部分？
...
1. **一次一问** - 不要用多个问题轰炸用户
2. **优先选择题** - 比开放式问题更容易回答
3. **YAGNI 原则** - 无情砍掉不必要的功能
4. **探索替代方案** - 确定方案前至少提 2-3 种选择
5. **渐进验证** - 分段呈现设计，逐段确认
6. **门控开发** - 设计确认前不进入实现阶段
7. **灵活回退** - 随时可以回头澄清
...
| 启动开发 | 触发 `speckit.plan` → `speckit.tasks` → `speckit.implement` |
| 调整设计 | 继续 Phase 3 分段讨论 |

**衔接点**：脑暴产出的设计文档可直接作为 `speckit.specify` 的输入。
